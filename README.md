...
1. Install the [aws_stackstorm-aws_elasticbeanstalk pack](https://github.com/stackstorm-exchange/stackstorm-aws_stackstorm-aws_elasticbeanstalk):

    ```
    # Install AWS
    st2 pack install aws_stackstorm-aws_elasticbeanstalk

    # Check it
    st2 action list -p aws_stackstorm-aws_elasticbeanstalk
    ```

2. Copy the example configuration in [aws.yaml.example](./aws.yaml.example)
to `/opt/stackstorm/configs/aws_stackstorm-aws_elasticbeanstalk.yaml` and edit as required. It must contain:
...
## st2_user_data (relevant to aws_ec2 only)
...
Prior to installation of the aws pack, you can get the list of available actions here:
  ...
  https://github.com/StackStorm-Exchange/stackstorm-aws_stackstorm-aws_elasticbeanstalk/tree/master/actions
...
```
root@2e1d15fd5d07:/# st2 action get aws_route53.list_hosted_zones
+-------------+--------------------------------------------------------------+
| Property    | Value                                                        |
+-------------+--------------------------------------------------------------+
| id          | 594bfa4ed5d05c0b7e504803                                     |
| uid         | action:aws_route53:list_hosted_zones                         |
| ref         | aws_route53:list_hosted_zones                                |
| pack        | aws_route53                                                  |
...
```
$ st2 run aws_route53.list_hosted_zones
.
...
```
source /opt/stackstorm/virtualenvs/aws_stackstorm-aws_elasticbeanstalk/bin/activate
cd /opt/stackstorm/packs/aws_stackstorm-aws_elasticbeanstalk/etc/st2packgen
python st2packgen.py -d /opt/stackstorm/packs/aws_stackstorm-aws_elasticbeanstalk/actions
st2ctl reload --register-actions
```

`st2packgen.py` will overwrite any existing actions at `/opt/stackstorm/packs/aws_stackstorm-aws_elasticbeanstalk/actions`.
